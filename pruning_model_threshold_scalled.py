...
def scalling(all_weight):
    all_sum=[]
    for i in range(len(all_weight)):
        all_sum.append(all_weight[i].weight[0].data.cpu().numpy().sum())
    
    max_sum=np.argmax(all_sum)
    
    scalled_weight=[]
    for i in range(len(all_weight)):
        scalled_weight.append(abs((all_weight[i].weight[0]-all_weight[max_sum].weight[0])+(all_weight[i].weight[0])))
        
    return scalled_weight
...
def weight_threshold(model, thr, encoder_layer_name, decoder_layer_name, pruning_part,pruning_weight_percentage):
    ...
    #print("Threshold from weight_threshold: ", thr)
    ...
    if str(pruning_part)== "decoder":
        decoder_layer=scalling(decoder_layer)

        for i in range(len(decoder_layer)):
            count_zero_filter=0


            for j in range (decoder_layer[i].size(0)):
                    if abs(decoder_layer[i][j].cpu().detach().numpy()).sum()< thr:
                        count_zero_filter+=1

                        #print("layer", decoder_layer[i].weight[0])
                        #print("filter", decoder_layer[i].weight[0][j])
                        with torch.no_grad():
                            decoder_layer[i][j,:,:]=0
                            #decoder_layer[i].weight[0][j]=0

            #print(decoder_layer[i].weight[j,:,:,:])
            #print("\n----------------\n",count_zero_filter)
            #print(decoder_layer[i].weight[0])
            count_zero_filter=0
...
def normal_distribution(layer):
    ...
    for i in range(len(layer)):
        ...
        layer_mean.append(np.mean(abs(tensor)))
        all_weight.append(abs(tensor))


    sum_of_filters=[]
    for i in range(len(layer)):
        for j in range (layer[i].weight[0].size(0)): #decoder_layer[i].weight[0].size(0), it gives number of out_channel in decoder layer
            sum_of_filters.append(abs(layer[i].weight[0][j].data.cpu().numpy()).sum())
    ...
    return all_weight, layer_mean, sum_of_filters




def layer_wise_encoder_decoder_pruning_filters_count(model, encoder_layer_name, decoder_layer_name, pruning_part,thr):
    ...
    '''
    encoder_layer,all_encoder_layer,decoder_layer,all_decoder_layer,layer_id=get_full_network_layer(model, encoder_layer_name, decoder_layer_name, pruning_part)
    ...
    if str(pruning_part)=="decoder":
        ...
        for i in range(len(all_decoder_layer)):

            if i==len(all_decoder_layer)-1:
                count_zero_filter=0
                zerod_filter_index["layer_"+str(i+1)]= count_zero_filter

            else:
                for j in range (all_decoder_layer[i].weight[0].size(0)):
                    if abs(all_decoder_layer[i].weight[0][j].cpu().detach().numpy()).sum()< thr:
                        count_zero_filter+=1
                        zerod_filter_index["layer_"+str(i+1)]= count_zero_filter
...
def layer_wise_encoder_decoder_pruning_filters_count_without_thr(model, encoder_layer_name, decoder_layer_name, pruning_part):
    ...
    if str(pruning_part)=="decoder":
        for i in range(len(all_decoder_layer)):
            sum_of_kernel = list(torch.sum(torch.abs(all_decoder_layer[i].weight[0].view(all_decoder_layer[i].weight[0].size(0), -1)),
                                           dim=1).cpu().detach().numpy())
            count_zero_filter=sum_of_kernel.count(0)
            zerod_filter_index["layer_"+str(i+1)]= count_zero_filter
...
def pruning_train_one_epoch(model, optimizer, criterion, loader, alpha=None, encoder_layer_name=None,
    ...
    for i, img in enumerate(loader):
        ...
        if apply_thr == "YES":
            weight_threshold(model, thr, encoder_layer_name, decoder_layer_name, pruning_part,pruning_weight_percentage)

            decoder_prunned_channel = layer_wise_encoder_decoder_pruning_filters_count_without_thr(
...
def train_for_pruning(model, train_loader, test_dataloader, val_loader, lbda=None, epochs=None,
    ...
    encoder_layer, _, decoder_layer, _, _ = get_full_network_layer(model, encoder_layer_name, decoder_layer_name,
                                                                   pruning_part)

    all_weight, layers_mean, filter_sum = normal_distribution(decoder_layer)
    ...
    #small_layer_weights=all_weight[small_mean].flatten()


    sort_weight=sorted(filter_sum)
    
    per=round(len(sort_weight)*pruning_weight_percentage)/100
 
    print(len(sort_weight))
    print(int(per))
    print(sort_weight[int(per)])


    thr1 = sort_weight[int(per)]
    ...
    model_1=pruning_train_one_epoch(model, optimizer, criterion, train_loader, alpha=alpha,
                            encoder_layer_name=encoder_layer_name, decoder_layer_name=decoder_layer_name,
                            pruning_part=pruning_part, regularizer=regularizer, thr=thr1, apply_thr="YES", pruning_weight_percentage=pruning_weight_percentage)
    ...
    psnr, bpp= performance_test(model_1, test_dataloader)
    print(f'\n PSNR: {psnr:.2f} | BPP: {bpp:.2f} \n\n')

    torch.save(model.state_dict(), './cp/pruned/'+directory+'/{}_best.tar.pth'.format("pruned_model_lambda_"+str(lbda)+"_alpha_"+str(alpha)))
...
def train_for_pruning(model, train_loader, test_dataloader, val_loader, lbda=None, epochs=None,
    ...
    '''
    pruning_train_one_epoch(model, optimizer, criterion, train_loader, alpha=alpha,
                            encoder_layer_name=encoder_layer_name, decoder_layer_name=decoder_layer_name,
                            pruning_part=pruning_part, regularizer=regularizer, thr=thr1, apply_thr="YES", pruning_weight_percentage=pruning_weight_percentage)
    '''
    ...
    decoder_prunned_channel = layer_wise_encoder_decoder_pruning_filters_count(
                net, encoder_layer_name="g_a", decoder_layer_name="g_s", pruning_part="decoder",thr=thr1)

    print("\n Decoder Pruned Channelwise: ", decoder_prunned_channel)

    weight_threshold(model, thr1, encoder_layer_name, decoder_layer_name, pruning_part,pruning_weight_percentage)

    decoder_prunned_channel = layer_wise_encoder_decoder_pruning_filters_count_without_thr(
                model, encoder_layer_name="g_a", decoder_layer_name="g_s", pruning_part="decoder")

    print("After pruning", decoder_prunned_channel)

   


    print(f'\n Total Pruning filters: {sum(list(decoder_prunned_channel.values()))}')

    print(f'\n Total Prunable filters: {count_prunable_filters(net,encoder_layer_name="g_a", decoder_layer_name="g_s", pruning_part="decoder")}')

    print(f'\n Percentage of Pruning: {(sum(list(decoder_prunned_channel.values()))*100)/count_prunable_filters(net,encoder_layer_name="g_a", decoder_layer_name="g_s", pruning_part="decoder"):.2f}')
    ...
    print("\n Decoder Pruned Channel after pruning: ", decoder_prunned_channel)

    torch.save(net,'./cp/pruned/'+directory+'/{}_best.tar.pth'.format
        ("pruned_model_lambda_"+str(lbda)+"_alpha_"+str(alpha)+"_per_"+str(pruning_weight_percentage)+"_thr_"+str(thr1)))
...
if __name__ == '__main__':
    ...
    if args.lbda == 0.0483:
        ...
        net = torch.load("./cp/regularized/0_0483/pruned_model_alpha_1e-05_0.0483_best.tar.pth")
    ...
    if args.pruning_part=="encoder":
        ...
        new_encoder_pruned_model_without_weight= partial_network_pruning_architecture_without_weights(
            model, pruned_channel, model.g_a, encoder_layer_name="g_a",
    ...
    if args.pruning_part=="decoder":
        ...
        decoder_prunned_channel= layer_wise_encoder_decoder_pruning_filters_count(
            model, encoder_layer_name="g_a", decoder_layer_name="g_s",pruning_part="decoder",thr=thr1)
        ...
        new_decoder_pruned_model_without_weight= partial_network_pruning_architecture_without_weights(
            model, pruned_channel, model.g_s, encoder_layer_name="g_a",
