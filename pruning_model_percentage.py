...
def weight_threshold(model, thr, encoder_layer_name, decoder_layer_name, pruning_part,pruning_weight_percentage):
    ...
    if str(pruning_part)== "decoder":
        ...
        for i in range(len(decoder_layer)):
            ...
            for j in range (decoder_layer[i].weight[0].size(0)-10):
                weights = decoder_layer[i].weight.data.cpu().numpy()
                weights=weights[:-10]
                len(weights)
                idx_out = abs(weights) <= thr
                weights[idx_out] = 0
                decoder_layer[i].weight.data = torch.from_numpy(weights).cuda()



            #print(f'layer:{i} | filter Pruned: {pruned_filter}')

            #if pruned_filter >= 110 or pruned_filter<=120:
                #print(pruned_filter)
                    

            #print("filter", pruned_filter)
            pruned_filter=0
...
def train_for_pruning(model, train_loader, test_dataloader, val_loader, lbda=None, epochs=None,
    ...
    all_weight = normal_distribution(decoder_layer)

    sort_weight=sorted(all_weight)
